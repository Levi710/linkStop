...
export function DomainCard({ domain, timeSlot, meetLink }: DomainCardProps) {
    ...
    // Check if the link is a placeholder or genuine
    const isPlaceholder = !meetLink || meetLink.includes("placeholder");

    // Only allow joining if we have a REAL link.
    // The user explicitly requested to hide the option if no actual link is attached.
    const canJoin = !isPlaceholder;
            ...
            <div className="mt-auto">
                {!canJoin ? (
                    <Button disabled variant="secondary" className="w-full opacity-50 cursor-not-allowed">
                        Wait for Schedule / Link
                    </Button>
                ) : (
                    <Link href={meetLink!} target="_blank" className="w-full block">
